...
class CircleChanger(object):
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        ################################################################
        circle = self.circle
        circle.radius = circle.radius + amount_to_swell_or_shrink
        if circle.radius < 1:
            circle.radius = 1
        else:
            pass
        circle.outline_thickness = random.randrange(3, 16)
        index = random.randrange(0, len(self.colors))
        circle.fill_color = self.colors[index]
    ...
    def swallow(self, other_circle_changer):
        ...
        #   NO CREDIT if you use the distance formula here.
        ################################################################
        dist = other_circle_changer.get_distance_from(self.circle.center)
        rad = dist / 2
        midx = ((other_circle_changer.circle.center.x - self.circle.center.x) / 2) + self.circle.center.x
        midy = ((other_circle_changer.circle.center.y - self.circle.center.y) / 2) + self.circle.center.y
        colors = self.colors + other_circle_changer.colors
        new = CircleChanger(midx, midy, rad, 'red', colors)
        return new
